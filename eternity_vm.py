...
class EternityVM:
    ...
    def seed_grid(self):
        """Seed the grid from the first N bytes of the loaded firmware."""
        idx = 0
    ...
    def run(self, cycles=1000):
        ...
        # The binary is both the "seed" and the "rule modifier"
        if not self.memory and not self.load_program():
            return
        self.seed_grid()
    ...
    def render(self):
        # Repaint in place via ANSI cursor-home so the grid animates
